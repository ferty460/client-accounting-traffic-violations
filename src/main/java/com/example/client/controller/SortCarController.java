...
public class SortCarController {
    ...
    void getAll() throws IOException {
        try {
            ...
            for (int i = 0; i < dataArr.size(); i++) {
                CarEntity newDriver = gson.fromJson(dataArr.get(i).toString(), CarEntity.class);
                carsData.add(newDriver);
            }
    ...
    void find(ActionEvent event) throws IOException {
        try {
            String res = http.get(api, "car/all_byDriver?id=" + driversList.getValue().getDriver_Id());
            System.out.println(res);
            JsonObject base = gson.fromJson(res, JsonObject.class);
            JsonArray dataArr = base.getAsJsonArray("data");
            for (int i = 0; i < dataArr.size(); i++) {
